(function() {
  ...
  LSM303 = (function() {
    ...
    LSM303.prototype.enableDefault = function() {
      // 00100 Normal / low-power mode (10 Hz)
      //111 => z/y/z enabled
      
      this._send(this.LSM303_CTRL_REG1_A, 0x27);
      if (this._device === this.LSM303DLHC_DEVICE) {
        this._send(this.LSM303_CTRL_REG4_A, 0x00);
        console.log('LSM303DLHC_DEVICE');
      }
    ...
    LSM303.prototype.readAcc = function() {
      var _this = this;
      this.accelerometerWire.readBytes((this.LSM303_OUT_X_L_A | 0x80), 6, function(err, res){
        if (err != null)
        {
          ...
          _this.accReading.x = _this.accelerationInMS2(res, 0)
          _this.accReading.y = _this.accelerationInMS2(res, 2)
          _this.accReading.z = _this.accelerationInMS2(res, 4)

          console.log('acceleration X (scaled): ' + _this.accReading.x); 
          console.log('acceleration Y (scaled): ' + _this.accReading.y); 
          console.log('acceleration Z (scaled): ' + _this.accReading.z); 

          var totAcceleration = Math.sqrt(_this.accReading.x * _this.accReading.x + _this.accReading.y * _this.accReading.y + _this.accReading.z * _this.accReading.z);

          console.log('Result acceleration: ' + totAcceleration); 
        }
      });  
    ...
    LSM303.prototype.accelerationInMS2 = function(buffer, index){
      var lowByte = buffer.readUInt8(index);
      var highByte = buffer.readUInt8(index + 1);

      if (highByte&0x80){
        highByte |= (-1 << 8);
      }
      var scaleFactor = 9.806 / 1024;
      acceleration = (highByte << 8 | lowByte) >> 4;     
      return acceleration * scaleFactor;
    };
